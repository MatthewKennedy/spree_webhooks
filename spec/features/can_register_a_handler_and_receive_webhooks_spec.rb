...
RSpec.feature "Can register a handler and receive Webhooks", type: :request do
  background do
    SpreeWebhooks::Config.register_webhook_handler :proc, proc_handler
    SpreeWebhooks::Config.register_webhook_handler :method, method_handler
    SpreeWebhooks::Config.register_webhook_handler :user, user_handler
    SpreeWebhooks::Config.register_webhook_handler :splat, splat_handler
    SpreeWebhooks::Config.register_webhook_handler :method_and_user, method_and_user_handler
  end
  ...
  let(:method_and_user_handler) {
    Struct.new(:payloads) {
      def call(payload, user)
        payloads << [payload, user]
      end
    }.new(method_and_user_payloads)
  }
